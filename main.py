...
with st.sidebar:
    ...
    edit_mode = st.toggle("Edit Mode", False)
    if edit_mode:
        st.write("Edit mode is on")

    else:
        st.write("Edit mode is off")
        # TODO: Show RGB Array , Operations, Grayscale
        ...
        # TODO: Show Statistics and Histogram
...
if edit_mode:
    ...
    if 'uploaded_image' in st.session_state:
        if st.session_state['new_image_uploaded']:
            display_image(
                st.session_state['uploaded_image'], caption="Uploaded Image", width=200, key="uploaded_image_edit")
            if image_edit_result:
                display_image(
                    image_edit_result, caption="Image Edit Result", width=200, key="image_edit_result")

        st.divider()
        # Edit features based on user selections
        feature = [
            "RGB Array",
